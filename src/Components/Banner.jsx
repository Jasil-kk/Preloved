...
const Banner = () => {
  ...
  return (
    <div className="w-full h-auto flex flex-wrap justify-center gap-10 px-10 pt-16 pb-24  sm:pt-28 sm:px-40 relative">
      {Array.from({ length: itemsToShow }).map((_, idx) => (
        <Link to={"/singlepage"} key={idx}>
          <div className="banner-card w-80 h-96 rounded-xl border border-gray-300 overflow-hidden relative">
            <span className="absolute right-3 top-3 text-3xl cursor-pointer z-10">
              <MdOutlineFavoriteBorder className="text-slate-500 hover:text-pink-500" />
            </span>
      ...
      <button
        onClick={toggle}
        className="absolute bottom-6 w-28 h-10 border-2 border-blue-400 rounded-xl drop-shadow-lg font-poppins text-blue-700 hover:bg-blue-400 hover:text-slate-50"
      >
        Show More
      </button>
    </div>
