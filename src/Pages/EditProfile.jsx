...
import { profileDeleteApi, profileUpdateApi, userProfileApi } from "../Store/AuthSlice";
    ...
    <>
      ...
      <div className="w-full h-auto py-10 bg-slate-200 flex justify-center items-center">
        {userData?.map((user, key) => (
          <div
            key={key}
            className="w-1/3 h-auto bg-slate-50 border border-slate-400"
          >
            <div className="flex justify-between p-3 border-b border-slate-400">
              <h1 className="text-2xl font-semibold">Edit Profile</h1>
              <button onClick={handleOpen} className=" w-auto h-10 px-3 text-lg text-red-500 rounded-md hover:border border-red-200">Delete Profile</button>
            </div>
            ...
            <div className="p-6 flex justify-between">
              <Link to={"/"}>
                <button
                  className="h-10 px-4 bg-slate-700 text-slate-50 text-lg rounded-md"
                >
                  Discard
                </button>
              </Link>
      ...
      >
        <Box sx={style}>
          <Typography id="modal-modal-description" sx={{ mt: 1, fontSize: 18, fontFamily:"poppins" }}>
